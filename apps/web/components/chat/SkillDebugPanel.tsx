...
export function SkillDebugPanel() {
  ...
  const title =
    selectedSkills.length > 0
      ? `Skill：候选 ${candidateCount} · 选中 ${selectedSkills.length}`
      : `Skill：候选 ${candidateCount} · 未选中`;

  return (
    <details className="group mx-auto w-full max-w-3xl rounded-lg border border-primary/20 bg-card/95 px-3 py-2 text-sm shadow-sm">
      <summary
        className={cn(
          "flex cursor-pointer list-none items-center gap-2 text-sm font-medium select-none",
      ...
      <div className="mt-2 space-y-2 text-xs text-muted-foreground">
        <div className="grid gap-1 sm:grid-cols-3">
          <SkillMetric label="候选 Skill" value={String(candidateCount)} />
          <SkillMetric label="Router 选中" value={String(selectedSkills.length)} />
          <SkillMetric label="Task Frame 强制注入" value={forcedCount > 0 ? "是" : "否"} />
        </div>

        {selectedSkills.length > 0 ? (
          <div className="space-y-2">
            {selectedSkills.map((skill) => (
              <div key={skill.id} className="rounded-md bg-muted/50 px-2 py-1.5">
